...
4. 需要开机自动登录时，双击 `Install.cmd`。它创建当前用户的登录计划任务，无额外延迟，在登录 Windows 时启动，无需管理员权限。旧版启动文件夹快捷方式会在任务安装成功后移除。
...
- 39 项本地回归检查覆盖密码编码、配置加密读写、套餐匹配、完整模拟登录流程、错误密码、在线设备上限、响应校验和计划任务定义。
